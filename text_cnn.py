...
class TextCNN(object):
    def __init__(self):
        with tf.variable_scope("embeddings"):
            self.embeddings = tf.get_variable('embeddings', [config.VOCAB_SIZE, config.EMBED_SIZE], dtype=tf.float32,
                                              initializer=tf.truncated_normal_initializer(stddev=0.01))

    def forward(self, input, dropout_keep_prob_ph):
        with tf.device('/cpu:0'):
            embed = tf.nn.embedding_lookup(self.embeddings, input, name='embedding_lookup')
